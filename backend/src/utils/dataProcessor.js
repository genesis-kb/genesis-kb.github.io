    ...
    const talk = {
      ...
      duration: row.duration_seconds != null ? (
        row.duration_seconds >= 3600
          ? `${Math.floor(row.duration_seconds / 3600)}:${Math.floor((row.duration_seconds % 3600) / 60).toString().padStart(2, '0')}:${(row.duration_seconds % 60).toString().padStart(2, '0')}`
          : `${Math.floor(row.duration_seconds / 60)}:${(row.duration_seconds % 60).toString().padStart(2, '0')}`
      ) : 'N/A',
      date: formattedDate,
